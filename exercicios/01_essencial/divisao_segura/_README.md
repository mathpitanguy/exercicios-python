...
A função recebe dois números, `a` e `b`, e deve retornar o resultado da divisão de `a` por `b`. Como a divisão por zero gera um erro, proteja a operação: se `b` for `0`, capture o erro e retorne `None`.
...
- `resposta(10, 2)` deve retornar `5.0`
- `resposta(9, 3)` deve retornar `3.0`
- `resposta(5, 0)` deve retornar `None`
