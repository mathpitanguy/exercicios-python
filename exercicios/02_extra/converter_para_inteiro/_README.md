...
A função recebe o parâmetro `texto` (uma string). Tente convertê-lo para um número inteiro com `int()`. Se a conversão for bem-sucedida, retorne o número inteiro. Se o texto não representar um inteiro válido, capture o erro e retorne `None`.

> Em um programa interativo, esse texto viria de `input()`, que sempre devolve uma `string`. Aqui, ele chega diretamente no parâmetro `texto`.
